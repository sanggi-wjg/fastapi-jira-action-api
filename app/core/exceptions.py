...
async def bad_request_handler(request: Request, e: BadRequest):
    return JSONResponse(
        ...
        content={"code": status.HTTP_400_BAD_REQUEST, "detail": f"Bad request, {e}"},
    )
...
async def jira_error_handler(request: Request, e: JIRAError):
    ...
    return JSONResponse(
        status_code=status.HTTP_400_BAD_REQUEST,
        content={"code": status.HTTP_400_BAD_REQUEST, "detail": "Jira Not authenticated"},
    )
...
async def validation_error_handler(request: Request, e: RequestValidationError):
    return JSONResponse(
        status_code=status.HTTP_400_BAD_REQUEST,
        content={"code": status.HTTP_400_BAD_REQUEST, "detail": f"{e.errors()}"},
    )
